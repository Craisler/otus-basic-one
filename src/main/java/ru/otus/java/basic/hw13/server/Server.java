...
public class Server {
    public static void main(String[] args) throws IOException {
        try (ServerSocket serverSocket = new ServerSocket(8088)) {
            while (true) {
                ...
                readRequest(socket);
                sendMessage("Введите ваше выражение в формате x + y, доступные математические вычисления операции +, -, *, /", socket.getOutputStream());
                while (true) {
    ...
    private static String calculate(String request) {
        ...
        try {
            ...
            int firstVal = Integer.parseInt(parser[0].trim());
            int secondVal = Integer.parseInt(parser[2].trim());
            int result;
